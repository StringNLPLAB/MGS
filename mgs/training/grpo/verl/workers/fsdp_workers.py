...
from verl.utils.reward_score.ifeval_reward import compute_score as compute_score_if
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config):
        ...
        # longcot config
        self.longcot_config = config.get('longcot_config', None)
        if self.longcot_config is not None:
            import json
            longcot_config = json.load(open(self.longcot_config))
            self.longcot_delimiter = longcot_config.get('longcot_delimiter', None)
            if self.longcot_delimiter == "<response>":
                self.end_delimiter = "</response>" 
            else:
                self.end_delimiter = longcot_config.get('end_delimiter', None)

            print(f"Longcot config: {self.longcot_delimiter}, {self.end_delimiter}")
        else:
            self.longcot_delimiter = None
            self.end_delimiter = None

    def _extract_response(self, seq: str):
        # Store original sequence for logging
        original_seq = seq

        # if there is no longcot delimiter then we don't think
        if self.longcot_delimiter is None:
            return seq
        elif self.longcot_delimiter in seq:
            seq = seq[seq.find(self.longcot_delimiter) + len(self.longcot_delimiter):].lstrip()
        if self.end_delimiter is not None and self.end_delimiter in seq:
            seq = seq[:seq.find(self.end_delimiter)].rstrip()
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            for micro_batch in micro_batches:
                rm_score = self._forward_micro_batch(micro_batch) # TODO: add data source info in the micro batch
...
class SequenceRewardModelWorker(Worker, DistProfilerExtension):
    """
    Reward model that computes a single score per sequence using AutoModelForSequenceClassification,
    then expands that score to token-level by placing it at the last valid token.
    """

    def __init__(self, config):
        Worker.__init__(self)
        DistProfilerExtension.__init__(self, DistProfiler(rank=self.rank, config=omega_conf_to_dataclass(config.get("profiler"))))
        ...
        # longcot config (optional)
        self.longcot_config = config.get("longcot_config", None)
        if self.longcot_config is not None:
            longcot_config = json.load(open(self.longcot_config))
            self.longcot_delimiter = longcot_config.get("longcot_delimiter", None)
            self.end_delimiter = longcot_config.get("end_delimiter", None)
            if self.end_delimiter is None and self.longcot_delimiter == "<response>":
                print(f"No end delimiter found, using default </response>")
                self.end_delimiter = "</response>"
        else:
            self.longcot_delimiter = None
            self.end_delimiter = None
    ...
    def _extract_response(self, seq: str) -> str:
        # Store original sequence for logging
        original_seq = seq
        ...
        if self.longcot_delimiter is None:
            ...
            # if there is no longcot delimiter, but we are in strict mode, then we nullify the response
            return "[NULL]"

        if self.end_delimiter is not None:
            if self.end_delimiter in seq:
                seq = seq[: seq.find(self.end_delimiter)].rstrip()
            elif self.strict:
                # if there is no end delimiter, but we are in strict mode, then we nullify the response
                return "[NULL]"
    ...
    def _build_model(self, config):
        ...
        if self.config.strategy == "fsdp":
            ...
            full_state = reward_module.state_dict()
            apply_fsdp2(reward_module, fsdp_kwargs, self.config.model.fsdp_config)
            fsdp2_load_full_state_dict(reward_module, full_state, fsdp_mesh, cpu_offload)
        else:
    ...
    #         )
    #         rm_score = output.logits  # (batch_size, 1)
    ...
    def _forward_micro_batch(self, micro_batch):
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            # Get base reward scores for ALL samples (fast, batched)
            output = self.reward_module(
                input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids
            )
            rm_score = output.logits.squeeze(-1)  # (batch_size,)
            
            # Only process math samples if they exist (lazy evaluation)
            # if "ability" in micro_batch and any(ability == "math" for ability in micro_batch["ability"]):
            abilities = micro_batch["ability"]
            # print("Abilities!: ", abilities)
            
            # Process math samples efficiently
            for i, ability in enumerate(abilities):
                if ability == "math":
                    # Get the response for this specific sample
                    response_tokens = input_ids[i:i+1]  # Keep as tensor for efficiency
                    response_text = self.tokenizer.decode(response_tokens[0], skip_special_tokens=True)
                    
                    math_reward = compute_score(
                        data_source="math",
                        solution_str=response_text,
                        ground_truth=micro_batch.get("answer", [""] * len(abilities))[i],
                        extra_info=micro_batch.get("extra_info", [{}] * len(abilities))[i],
                        method="strict"
                    )
                    # Replace RM score with math verification score
                    rm_score[i] = math_reward
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        rm_position_ids = compute_position_id_with_mask(rm_attention_mask)
        rm_inputs = {"input_ids": rm_input_ids, "attention_mask": rm_attention_mask, "position_ids": rm_position_ids}
        rm_data_source = {**data.non_tensor_batch, **rm_inputs}
        return DataProto.from_dict(rm_inputs)
...
class MixedRewardModelWorker(Worker, DistProfilerExtension):
    """
    Reward model that computes a single score per sequence using AutoModelForSequenceClassification,
    then expands that score to token-level by placing it at the last valid token.
    """
    ...
    def _extract_thought(self, seq: str):
        return seq.split("</think>")[0].replace("<think>", "")
    ...
    def _forward_micro_batch(self, micro_batch):
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            attention_mask = micro_batch["attention_mask"]
            position_ids = micro_batch["position_ids"]
            # responses = micro_batch["response"]
            ...
            for i, ability in enumerate(abilities):
                if ability == "math":
                    ...
                    math_reward = compute_score(
                        data_source="math",
                        solution_str=response_text,
                        ground_truth=micro_batch.get("ground_truth")[i],
                        extra_info=micro_batch.get("extra_info", [{}] * len(abilities))[i],
                        method="strict"
                    )
                    print("Math Reward: ", math_reward)
                    # Replace RM score with math verification score
                    rm_score[i] = math_reward
                if ability == "ifeval":
                    # Get the response for this specific sample
                    response_tokens = input_ids[i:i+1]
                    response_text = self.tokenizer.decode(response_tokens[0], skip_special_tokens=True)
                    
                    # 提取纯文本回复
                    # 建议加个 try-except 或者判断，防止 split 失败导致空字符串
                    if "<response>" in response_text:
                        response_text = response_text.split("<response>")[-1].replace("</response>", "")
                    if_reward = compute_score_if(
                        data_source="ifeval",
                        solution_str=response_text,
                        ground_truth=json.loads(micro_batch.get("ground_truth")[i]),
                        extra_info=None,
                        method="strict"
                    )
                    print("IF Reward: ", if_reward)

                    rm_score[i] = if_reward
            return rm_score
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            num_thought_tokens.append(len(self.tokenizer.encode(thought, add_special_tokens=False)))
            num_answer_tokens.append(len(self.tokenizer.encode(response, add_special_tokens=False)))
        ...
        rm_inputs = {"input_ids": rm_input_ids, "attention_mask": rm_attention_mask, "position_ids": rm_position_ids}
        rm_data_source = {**data.non_tensor_batch, **rm_inputs}
        return_data = DataProto.from_dict(rm_inputs)
        return_data.non_tensor_batch = data.non_tensor_batch
        return_data.non_tensor_batch['num_thought_tokens'] = num_thought_tokens
        return_data.non_tensor_batch['num_answer_tokens'] = num_answer_tokens
        # return_data.non_tensor_batch['response'] = response
        # print("Data is: ", return_data)
        return return_data
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Extract the non-tensor data you need BEFORE splitting
            abilities = rm_data.non_tensor_batch.get("ability")
            # responses = rm_data.non_tensor_batch.get("response")
            ground_truths = rm_data.non_tensor_batch.get("answer")
            batch_size = len(rm_data.batch["input_ids"])
            print("Abilities: ", abilities)
            ...
            if use_dynamic_bsz:
                ...
                micro_batches = []
                indices = []
                for i in range(0, batch_size, self.config.micro_batch_size_per_gpu):
                    end_idx = min(i + self.config.micro_batch_size_per_gpu, batch_size)
                    micro_batch = {k: v[i:end_idx] for k, v in rm_data.batch.items() if isinstance(v, torch.Tensor)}
                    micro_batches.append(micro_batch)
                    indices.append(list(range(i, end_idx)))
            print("Indices:\n")
            print(indices)
            # Add non-tensor data to each micro-batch
            for i, micro_batch in enumerate(micro_batches):
                idx_range = indices[i]
                micro_batch["ability"] = [abilities[idx] for idx in idx_range]
                # micro_batch["response"] = [responses[idx] for idx in idx_range]
                micro_batch["ground_truth"] = [ground_truths[idx] for idx in idx_range]
            ...
            token_level_scores = self._expand_to_token_level(data, scores)
            output = DataProto.from_dict(tensors={"rm_scores": token_level_scores})
            output.non_tensor_batch = rm_data.non_tensor_batch
